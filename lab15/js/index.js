document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', (event) => {
        ...
        if (isEditing && currentEditedElement) {
            // --- ВИКОНАНЕ ЗАВДАННЯ ---    
            // Оновлюємо текстовий вміст елементів у поточній картці
            ...
            // Оновлюємо дедлайн (якщо порожній — пишемо "Без дати")
            const deadlineEl = currentEditedElement.querySelector('.task-card__deadline');
            deadlineEl.textContent = taskData.deadline || 'Без дати';
            
            // Повертаємо форму у початковий стан
            resetFormState();
        } else {
