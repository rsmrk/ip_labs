...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchTodos() {
    ...
    try {
      ...
      list.innerHTML = "";
      // Фільтруємо, щоб бачити лише свої завдання (опційно, але корисно для тестування)
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (isEditing && currentEditId) {
      // --- ЗАВДАННЯ 2: Збереження змін (PUT) [cite: 436-441] ---
      try {
        ...
        if (!response.ok) throw new Error("Помилка при оновленні запису");

        await fetchTodos(); // Оновлюємо список
        resetFormState();   // Скидаємо форму
      } catch (error) {
  ...
  list.addEventListener("click", async (event) => {
    ...
    if (btn.classList.contains("btn--delete")) {
      if (confirm("Видалити цю задачу з сервера?")) {
        // --- ЗАВДАННЯ 1: Видалення (DELETE) [cite: 432-435] ---
        try {
          ...
          card.remove(); // Видаляємо картку з DOM
        } catch (error) {
